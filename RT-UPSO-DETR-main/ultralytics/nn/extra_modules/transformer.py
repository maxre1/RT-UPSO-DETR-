...
from .monaattention import Mona


__all__ = ['Cognitive_Spatial_Attention_Layer']
    ...
    def __init__(self, dim, hw, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,
        ...
        if sr_ratio > 1:
            self.sr = nn.Sequential(
                nn.Conv2d(dim, dim, kernel_size=sr_ratio, stride=sr_ratio, groups=dim),  # 深度卷积（参数减半）
                nn.AvgPool2d(kernel_size=2, stride=2) if sr_ratio > 2 else nn.Identity(),  # 二次下采样（仅当sr_ratio>2时启用）
                nn.Conv2d(dim, dim, kernel_size=1)  # 点卷积（通道融合）
            )
...
class Cognitive_Spatial_Attention_Layer(nn.Module):
    """Defines a single layer of the transformer encoder."""

    def __init__(self, c1, cm=2048, num_heads=8, dropout=0.0, act=nn.GELU(), normalize_before=False):
        """Initialize the TransformerEncoderLayer with specified parameters."""
        super().__init__()
        self.pola_attention = PolaFormer(c1, (20, 20))
